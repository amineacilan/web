...
(function (H) {
	...
	seriesTypes.treemap = extendClass(seriesTypes.scatter, merge(colorSeriesMixin, {
		...
		*/
        ...
        var biggest = 0;
        var smallest = 999999999999999999999999999;
        myTotal = 0;
        each(node.children, function (child) {
          if (child.level - 1 === levelRoot && node.isVisible)
          {   
            if(child.val > biggest)
            {
              biggest = child.val;
            } 
        ...
        if (child.level - 1 === levelRoot && node.isVisible)
        {
          point.color = nodeColorValues(child.val, myTotal, smallest);
        }
				...
				if (point.node.isVisible) {
					...
					if (!point.isLeaf) {
						if (pick(seriesOptions.interactByLeaf, !seriesOptions.allowDrillToNode)) {
							...
							attr.fill = Color(attr.fill).setOpacity(1).get();
							hover.fill = Color(hover.fill).setOpacity(0.7).get();
						}
					}
...
function nodeColorValues(nodeValue, totalValue, smallest)
{
  /*var colorR, colorG,
  color,
  precision = 100;
  
  /*if (nodeValue >= halfTotalValue)
  {
    colorRgb = parseInt((255 * nodeValue) / totalValue);
    color = 'rgb(255, '+ (255-colorRgb) +', 0)';
  }
  else if (nodeValue === smallest)
  {
    color = 'rgb(0, 255, 0)';
  }
  else
  {
    colorRgb = parseInt(((255 * nodeValue) / totalValue));
    color = 'rgb('+ colorRgb +', 255, 0)';
  }*/
  /*if (nodeValue === smallest)
  {
    color = 'rgb(0, 255, 0)';
  }
  else
  {*/
   /* colorR = parseInt(((255 * nodeValue) / totalValue));
    colorG = parseInt(((255 * (totalValue - nodeValue)) / totalValue));
    color = 'rgb('+ colorR +', '+ colorG +', 0)';*/
  /*}*/

 var val = parseInt((nodeValue*100)/totalValue),
